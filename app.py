...
def _init_voice():
    ...
    try:
        ...
        voice = VoiceCommands(
            ...
            wake_word="robin",  # Say "robin" then command
            arm_seconds=2.5
        )
        voice.start()
...
def main():
    ...
    while True:
        ...
        # FPS overlay (holographic style)
        fps_text = f"FPS: {fps_smooth:5.1f}"
        cv2.putText(composed, fps_text, (12, composed.shape[0] - 12),
                   cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 245, 0), 3, cv2.LINE_AA)
        cv2.putText(composed, fps_text, (12, composed.shape[0] - 12),
                   cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 128), 2, cv2.LINE_AA)
